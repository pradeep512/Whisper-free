...
class FileTranscribePanel(QWidget):
    ...
    def _connect_queue_signals(self):
        """Connect queue manager signals for thread-safe UI updates."""
    ...
    def _on_job_progress(self, job_id: str, progress: int):
        """Route queue progress signal for the current job only."""
        if job_id != self.current_job_id:
            return
        self._on_queue_progress(progress)

    def _on_job_completed(self, job_id: str, result_text: str, result_data: dict):
        """Route queue completion signal for the current job only."""
        if job_id != self.current_job_id:
            return
        self._on_queue_complete(result_text, result_data)

    def _on_job_failed(self, job_id: str, error_message: str):
        """Route queue failure signal for the current job only."""
        if job_id != self.current_job_id:
            return
        self._on_transcription_failed(error_message)
